...
from models import Net, NetC, NetSO2, NetTMBasis
...
def main():
    ...
    parser.add_argument('--data_noise',      dest='data_noise',      type=float,          default=0.0,                help='How much noise to add to the data.')
    parser.add_argument('--num_bases',       dest='num_bases',       type=int,            default=1061,                help='Option to limit the number of bases used in the TMBases model.')
    ...
    class MMFDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            speckle_max = np.max(np.abs(speckled_imgs))

            if self.noise != 0.0:
                speckled_imgs[speckled_imgs > speckle_max*0.9] = speckle_max*0.9
#                 print(speckled_img.dtype)
                img_noise = channels_to_complex_np(np.stack([np.random.normal(size=speckled_imgs.shape).astype(np.float32)*self.noise,np.random.normal(size=speckled_img.shape).astype(np.float32)*self.noise], axis=-1))
#                 print(img_noise.dtype)
                speckled_imgs = speckled_imgs + img_noise
    ...
    if args.dataset_name=='Real_fmnist':
        ...
        normalise_orig = True
    train_dataset = MMFDataset(speckled_imgs=speckled_imgs, original_imgs=original_imgs, normalise_orig=normalise_orig, rand_phase=args.rand_phase, noise=args.data_noise)
    test_dataset = MMFDataset(speckled_imgs=speckled_imgs_test, original_imgs=original_imgs_test, normalise_orig=normalise_orig, rand_phase=args.rand_phase, noise=args.data_noise)
    ...
    if args.model == 'cmlp':
        ...
        if args.dataset_name=='TheoryTM_fmnist':
            mod_1 = NetTMBasis(TM_name='TheoryTM_28', TM_name_in='TheoryTM', tm_full_mat=tm_full_mat, block_diag_mat=args.block_diag_mat), num_bases=args.num_bases
        if args.dataset_name=='TheoryTM_fmnist_v3':
            mod_1 = NetTMBasis(TM_name='TheoryTM_28', TM_name_in='TheoryTM', tm_full_mat=tm_full_mat, block_diag_mat=args.block_diag_mat, num_bases=args.num_bases)
        elif args.dataset_name=='TheoryTM_imagenette_grey':
            mod_1 = NetTMBasis(TM_name='TheoryTM_256', tm_full_mat=tm_full_mat, block_diag_mat=args.block_diag_mat, num_bases=args.num_bases)
        elif args.dataset_name=='TheoryTM_imagenette_grey_v2':
            mod_1 = NetTMBasis(TM_name='TheoryTM_256', tm_full_mat=tm_full_mat, block_diag_mat=args.block_diag_mat, num_bases=args.num_bases)
        elif args.dataset_name=='Real_fmnist':
            mod_1 = NetTMBasis(TM_name='TheoryTM_MM_28', TM_name_in='TheoryTM_MM_224', Real_in=True, tm_full_mat=tm_full_mat, block_diag_mat=args.block_diag_mat, num_bases=args.num_bases)
    mod_1 = mod_1.to('cuda')
    if args.dataset_name=='Real_fmnist':
